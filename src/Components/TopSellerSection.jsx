...
export default function TopSellers() {
  ...
  return (
    <div className="py-5 my-10">
      <h2 className="text-xl pl-3 font-bold text-left mb-6">Top Sellers</h2>
      <div className="relative flex items-center justify-center">
        {/* Slider Control - Previous */}
        <button
          onClick={prevSlide}
          className="absolute left-0 bg-white rounded-full shadow p-2 z-10"
        >
          <RiArrowRightLine className="text-2xl" />
        </button>

        {/* Product Cards */}
        <div className="flex gap-8 overflow-hidden p-4 w-full justify-center">
          {products.map((product, index) => (
            <div
              key={index}
              className={`relative rounded-lg shadow-md p-4 w-[250px] h-[300px] transition-transform duration-300 transform overflow-hidden  hover:shadow-lg hover:scale-105 group`}
            >
              <img
                src={png1}
                alt={product.name}
                className="mx-auto w-25 object-cover mb-6"
              />
        ...
        <button
          onClick={nextSlide}
          className="absolute right-0 bg-white rounded-full shadow p-2 z-10"
        >
